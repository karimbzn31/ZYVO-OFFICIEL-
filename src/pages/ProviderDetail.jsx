...
import { ArrowLeft, Star, ShieldCheck, MapPin, Calendar, Award, ChevronRight, CheckCircle, Clock } from 'lucide-react'
    ...
    <div className="py-8 max-w-3xl mx-auto">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row items-start gap-6">
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-1.5 text-xs text-zyvo-muted mt-3">
              <MapPin className="w-3.5 h-3.5 text-zyvo-gold" /> Alger, Algérie
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="glass-premium rounded-2xl p-6 mt-6">
        ...
        <p className="text-sm text-zyvo-muted leading-relaxed">
          Professionnel qualifié et vérifié avec <strong className="text-white">{provider.missions} missions</strong> réalisées
        ...
        <div className="grid grid-cols-2 gap-4 mt-6">
          {[
            { label: 'Missions', value: `${provider.missions}+` },
            { label: 'Note', value: `${provider.rating}/5` },
            { label: 'Membre depuis', value: '2025' },
            { label: 'Taux de réponse', value: '< 30 min' },
          ].map((stat) => (
      ...
      {bookingStep === 'idle' && (
        <div className="grid sm:grid-cols-2 gap-4 mt-6">
          <button
            onClick={handleBook}
            className="gradient-brand text-white font-bold py-4 rounded-2xl shadow-lg hover:scale-[1.02] transition-all duration-300 flex items-center justify-center gap-2 glow-worm"
          >
            <Calendar className="w-5 h-5" /> Réserver maintenant
          </button>
          <button className="glass-premium text-white font-bold py-4 rounded-2xl hover:bg-white/10 transition-all duration-300 flex items-center justify-center gap-2">
            <span className="text-lg">💬</span> Contacter
          </button>
        </div>
      )}
      ...
      <div className="glass-premium rounded-2xl p-6 mt-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-xs text-zyvo-muted">Basé sur {provider.missions} missions</span>
        </div>
        ...
        {[1, 2].map((review) => (
          <div key={review} className="border-t border-white/5 pt-4 mt-4 first:border-0 first:pt-0 first:mt-0">
            <div className="flex items-center gap-3">
              <div className={`w-8 h-8 rounded-full bg-gradient-to-br ${review === 1 ? 'from-blue-500 to-cyan-400' : 'from-purple-500 to-pink-400'} flex items-center justify-center text-xs font-bold text-white`}>
                {review === 1 ? 'S' : 'M'}
              </div>
              <div>
                <div className="font-bold text-sm text-white">{review === 1 ? 'Sarah K.' : 'Mohamed L.'}</div>
                <div className="flex items-center gap-1 text-xs text-zyvo-muted">
                  ...
                  <Star className="w-3 h-3 fill-zyvo-gold text-zyvo-gold" />
                  <Star className="w-3 h-3 fill-zyvo-gold text-zyvo-gold" />
                  <span className="ml-1">Il y a 2 semaines</span>
                </div>
              </div>
            </div>
            <p className="text-sm text-zyvo-muted mt-2 leading-relaxed">
              {review === 1
                ? 'Excellent service ! Professionnel, ponctuel et très compétent. Je recommande vivement.'
                : 'Très satisfait du travail. Prix correct et travail bien fait. Je ferai appel à nouveau.'}
            </p>
          </div>
        ))}
      </div>
