...
export default function ProviderDemandes() {
  ...
  const myQuotes = useMemo(() => {
    ...
    return quotes.filter(q =>
      q.category === provider.category || q.city === provider.city
    )
  }, [provider])

  const filtered = useMemo(() => {
    if (filter === 'pending') return myQuotes.filter(q => q.status === 'pending')
    if (filter === 'answered') return myQuotes.filter(q => q.status === 'answered')
    return myQuotes
  }, [myQuotes, filter])
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="font-bold text-sm text-white truncate">{q.title}</h3>
                    {q.urgency !== 'normal' && (
                  ...
                  <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px] text-zyvo-muted">
                    ...
                    <span className="flex items-center gap-0.5">
                      <DollarSign className="w-3 h-3" /> {q.budgetRange}
                    </span>
                    <span className="flex items-center gap-0.5">
                ...
                <div className="text-right shrink-0">
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${
                    q.status === 'pending' ? 'text-amber-400 bg-amber-500/10' :
                    q.status === 'answered' ? 'text-emerald-400 bg-emerald-500/10' :
                    'text-red-400 bg-red-500/10'
                  }`}>
                    {q.status === 'pending' ? 'Ouvert' : q.status === 'answered' ? 'Répondu' : 'Expiré'}
                  </span>
                </div>
              </div>
              ...
              {q.responses && q.responses.length > 0 && (
                <div className="mt-3 pt-3 border-t border-white/5">
                  ...
                  {q.responses.map(r => (
                    <div key={r.providerId} className="flex items-center gap-2 text-xs text-zyvo-muted mb-1">
                      <div className="w-5 h-5 rounded-md bg-white/5 flex items-center justify-center text-[9px] font-bold text-white">
                        {r.providerAvatar}
                      </div>
                ...
                <div className="mt-3 pt-3 border-t border-white/5">
                  {responding === q.id ? (
                    <div className="space-y-2">
                      ...
                      />
                      <div className="flex gap-2">
                        <button className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl bg-emerald-500/20 text-emerald-400 text-xs font-bold hover:bg-emerald-500/30 transition-all">
